...
LANG = {
    "English": {"title": "📓 Bharat Kisan NOTEBOOK", "new": "➕ Add New Crop", "cat": "Category", "crop": "Select Crop", "custom": "Add Your Own Crop Name", "area": "Area (Guntha)", "bhav": "Market Rate Rs/Q", "kharch": "💸 All Charges Per Acre", "beej": "Seed", "khat": "Fertilizer", "aushad": "Pesticide", "majdoor": "Labour", "pani": "Irrigation", "other": "Other", "weight": "📦 Total Yield", "gram": "Gram", "kg": "KG", "quintal": "Quintal", "ton": "Ton", "profit": "✅ Net Profit", "total_cost": "💸 Total Cost", "total_income": "💰 Total Income", "save": "💾 Save Page", "pages": "📚 My Pages", "weather": "🌦️ Weather & Mandi", "loan": "🏦 Loan Calculator", "edit": "✏️ Edit", "delete": "🗑️ Delete", "download": "📥 Download CSV"},
    "Marathi": {"title": "📓 Bharat Kisan NOTEBOOK", "new": "➕ नवीन पीक जोडा", "cat": "प्रकार", "crop": "पीक निवडा", "custom": "तुमचे स्वतःचे पीक नाव लिहा", "area": "क्षेत्र (गुंठा)", "bhav": "बाजार भाव Rs/Q", "kharch": "💸 सर्व खर्च प्रति एकर", "beej": "बीज", "khat": "खत", "aushad": "औषध", "majdoor": "मजूर", "pani": "पाणी", "other": "इतर", "weight": "📦 एकूण उत्पादन", "gram": "ग्रॅम", "kg": "किलो", "quintal": "क्विंटल", "ton": "टन", "profit": "✅ निव्वळ नफा", "total_cost": "💸 एकूण खर्च", "total_income": "💰 एकूण उत्पन्न", "save": "💾 जतन करा", "pages": "📚 माझी पाने", "weather": "🌦️ हवामान व बाजार", "loan": "🏦 कर्ज कॅल्क्युलेटर", "edit": "✏️ बदल करा", "delete": "🗑️ हटवा", "download": "📥 CSV डाउनलोड"},
    "Hindi": {"title": "📓 Bharat Kisan NOTEBOOK", "new": "➕ नई फसल जोड़े", "cat": "श्रेणी", "crop": "फसल चुने", "custom": "अपनी फसल का नाम लिखें", "area": "क्षेत्र (गुंठा)", "bhav": "बाजार भाव Rs/Q", "kharch": "💸 सभी खर्च प्रति एकड़", "beej": "बीज", "khat": "खाद", "aushad": "दवा", "majdoor": "मजदूर", "pani": "पानी", "other": "अन्य", "weight": "📦 कुल उपज", "gram": "ग्राम", "kg": "किलो", "quintal": "क्विंटल", "ton": "टन", "profit": "✅ शुद्ध लाभ", "total_cost": "💸 कुल खर्च", "total_income": "💰 कुल आमदनी", "save": "💾 सेव करे", "pages": "📚 मेरे पेज", "weather": "🌦️ मौसम व मंडी", "loan": "🏦 लोन कैलकुलेटर", "edit": "✏️ बदलें", "delete": "🗑️ हटाएं", "download": "📥 CSV डाउनलोड"}
}

lang = st.sidebar.selectbox("🌐 Language / भाषा", list(LANG.keys()), index=1)
T = LANG[lang]

# CATEGORY TRANSLATION - Box ke andar ke words
CAT_TRANS = {
    "English": ["All", "Vegetable", "Fruit", "Kharif", "Rabi", "Other"],
    "Marathi": ["सर्व", "भाजीपाला", "फळे", "खरीप", "रब्बी", "इतर"],
    "Hindi": ["सभी", "सब्जी", "फल", "खरीफ", "रबी", "अन्य"]
}
CAT_MAP = {"सर्व":"All", "भाजीपाला":"Vegetable", "फळे":"Fruit", "खरीप":"Kharif", "रब्बी":"Rabi", "इतर":"Other", "सभी":"All", "सब्जी":"Vegetable", "फल":"Fruit", "खरीफ":"Kharif", "रबी":"Rabi", "अन्य":"Other"}

# CROPS WITH HINDI/MARATHI NAMES + MORE FRUITS
CROPS_DB = {
...
with tab1:
    if not st.session_state.edit_id:
        ...
        # 1. CATEGORY - Ab Marathi me dikhega
        cat_display = st.selectbox(T["cat"], CAT_TRANS[lang])
        cat_eng = CAT_MAP.get(cat_display, cat_display)

        # 2. CROP LIST - Language ke hisab se
        def get_crop_name(k):
            d=CROPS_DB[k]
            if lang=="Marathi": return f"{d['mr']} ({k})"
            if lang=="Hindi": return f"{d['hi']} ({k})"
            return k

        filtered = [k for k,v in CROPS_DB.items() if cat_eng=="All" or cat_eng=="Other" or v["cat"]==cat_eng]
        crop_options = [get_crop_name(k) for k in filtered] + [f"✏️ {T['custom']}"]

        crop_sel = st.selectbox(T["crop"], crop_options)

        # 3. KISAN KHUD BHAR SAKE
        if T["custom"] in crop_sel:
            custom_crop = st.text_input(T["custom"], "Meri Fasal")
            crop_key = custom_crop
            data = {"yield": 50, "beej": 5000, "khat": 5000, "aushad": 5000, "majdoor": 8000, "pani": 2000, "other": 1000}
            final_crop_name = custom_crop
        else:
            # English key nikalo
            crop_key = crop_sel.split("(")[-1].replace(")","").strip() if "(" in crop_sel else crop_sel
        ...
        # 4. WEIGHT - GRAM, KG, QUINTAL, TON - KISAN CHANGE KAR SAKE
        st.subheader(T["weight"] + " - Unit Badal Sakte Ho")
        y1,y2 = st.columns([2,1])
        with y1:
            yield_input = st.number_input(f"{T['weight']} Quantity", value=float(data["yield"]), min_value=0.0)
        with y2:
            unit_choice = st.selectbox("Unit Select Karo", [T["gram"], T["kg"], T["quintal"], T["ton"]])

        unit_map = {T["gram"]:0.00001, T["kg"]:0.01, T["quintal"]:1.0, T["ton"]:10.0, "Gram":0.00001, "KG":0.01, "Quintal":1.0, "Ton":10.0, "ग्रॅम":0.00001, "किलो":0.01, "क्विंटल":1.0, "टन":10.0, "ग्राम":0.00001}
        ...
        with c_cancel:
            if st.button("⬅️ Back", use_container_width=True): st.session_state.edit_id=None; st.rerun()
